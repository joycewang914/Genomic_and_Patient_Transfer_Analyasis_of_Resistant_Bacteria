# Plot Figure S11 showing the correlation between the popoulation structure of each antibiotic-resistant lineage and geographical distance between nursing facilities.
...
for (org in names(inter_NH_Fsp_list)){
    ...
    pt_model = lm(final_df$fsp ~ final_df$geodist)
    
    plot(final_df$geodist, final_df$fsp, pch = 16, 
         ylab = "Isolate similarity between NFs\n(low:similar; high:different)", 
         xlab = "Geographical distance\nbetween NFs (kilometer)", col= cols[org])
    
    abline(pt_model, col = cols[org], lwd = 3)
    text(final_df$fsp ~ final_df$geodist, labels=pair,data=final_df, cex=0.75, font=2)
